...
ffprobe. It writes `results.mp4`, `report.md`, and `manifest.json` into the
session folder.
...
`~/.claude/skills/test-with-evidence/scripts/evidence.py`). It needs only
Python 3 and FFmpeg.
...
- **Crash-safe raw capture**: the raw recording is MPEG-TS (`raw.ts`), so if
  ...
  renders. `stop` remuxes or re-encodes it into a standard `results.mp4`.
